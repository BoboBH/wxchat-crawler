...
# 可行性尖峰F —— 公网直取文章与 Edge 无头打印成 PDF(2026-09-05)

- **环境**: Windows 11 / Edge(版本化 msedge.exe)/ Python 3.12(venv)
- **背景**: 用户想「每篇文章转成一个 PDF」,先后提议「···→打印 Ctrl+P」与
  自研无头浏览器。本尖峰验证公网取文与无头打印的真实可行性。**结论:用户
  已有完整 URL→PDF 程序,爬虫侧不集成 PDF**,本文仅归档实测结论。
- **Gate F 结论**: 裸 urllib 已被拦;真 Chromium 指纹能过但**频率敏感**
  (连发第二发即验证码);「微信内打印对话框」是模态对话框链,不采用;
  产出物等价物 = `msedge --headless=new --print-to-pdf`,一行命令零对话框。

## 实测要点

| 实验 | 结果 |
| --- | --- |
| urllib GET canonical(浏览器 UA + gzip) | 302 回原 URL →「未知错误,请稍后再试」错误页(17KB,无 js_content,也非验证码页)。对照尖峰C(09-04 同法可得 3.5MB 全文):**门槛是间歇性/状态型,非固定规则** |
| Edge 无头首访同 URL | **过**:完整 8 页 1.2MB PDF(标题《货币的秩序》/作者/日期/正文配图齐全) |
| Edge 无头连续第二发(数秒内) | **秒拦**:`wappoc_appmsgcaptcha` 验证码页(1 页 31KB;PDF 文本里带 captcha URL,可据此做产物检测) |
| `msedge --dump-dom` | stdout 不可靠(GUI 子系统 + 启动器脱钩);验内容一律用 `--print-to-pdf` 落文件再解析(pypdf 验过即卸) |
| 启动器行为 | msedge.exe 只是启动器,无头 worker **脱钩运行** → 判完成只能轮询「PDF 文件出现且大小连续两次采样稳定」,不能只等进程退出;文件出现可能滞后十几秒 |
| 微信「···」菜单的「打印 Ctrl+P」 | 打印预览 → 打印对话框 → 另存为对话框的模态链,批量自动化会把 AppEx 劫持、还牵动系统打印机全局状态 → 不采用 |

## 对下游(URL→PDF 自研程序)有用的定律

1. **限速是生死线**:同一指纹两次连续请求即触发验证码;批量抓取必须拉开
   间隔(阈值未标定,45s 量级起测),失败退避重试;
2. 拦截页文本特征(渲染产物里可识别):`wappoc_appmsgcaptcha` / `环境异常`
   / `未知错误` / `操作频繁`;
3. 可用的无头打印参数组合:`--headless=new --no-pdf-header-footer
   --virtual-time-budget=15000`(等懒加载图)+ **独立 `--user-data-dir`**
