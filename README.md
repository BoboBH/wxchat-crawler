...
- **前台被占用**:正在用电脑时微信抢不到前台,搜索框粘贴会先经前台闸门
  (自动置前+复核,2026-09-11 起);置前失败则整轮账号跳过不发键
  (日志:`AppEx 窗口未置前,不发合成键` / `粘贴前前台被抢且置回失败`),
  锁屏则报 `桌面状态=locked…不发合成键` → 当轮账号失败;
  手动跑全量轮请选电脑空闲时段。
...
- **前台竞争**:合成键鼠(搜索粘贴/复制链接菜单)要求微信能拿到前台。
  2026-09-11 起搜索粘贴会自动置前并复核,置不回则跳过该账号、绝不把
  Ctrl+A/V 误发进用户正在用的应用;复制链接菜单仍是树下提取兜底。
