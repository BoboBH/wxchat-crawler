...
的新文章逐篇打开,从文章页无障碍树的 ValuePattern 提取 canonical URL
(多候选或归属存疑时,改用页面「··· → 复制链接」菜单取页面自身链接)→
入库;抓完即关文章页签,轮末兜底清扫。不注入、不逆向、不需要公众号后台
权限;行为等价于真人浏览。
...
- **桌面需解锁**:文章页提取全程可用 UIA 完成(锁屏也行),但搜索框
  粘贴与「复制链接」菜单依赖合成键鼠 —— 锁屏时段运行的轮次中,账号会
  失败并留日志,下一轮自动重试。
